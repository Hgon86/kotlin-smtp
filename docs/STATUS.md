...
- ETRN 도메인 인자 검증/정규화
  - `ETRN <domain>` 인자를 필수로 강제하고 IDNA ASCII 도메인으로 정규화
  - 빈 인자/유효하지 않은 도메인은 501(syntax)로 거부
- 도메인 인자 전달 경계 추가
  - `SmtpDomainSpooler` 확장 훅을 추가해, 도메인 인자를 지원하는 스풀러는 도메인 단위 트리거 가능
  - 미지원 스풀러는 기존 동작(전체 큐 트리거)으로 폴백
- starter 스풀러 도메인 처리
  - `MailSpooler`가 도메인 지정 트리거를 구현하고, 지정 도메인 수신자만 우선 처리
- 통합 테스트 보강
  - `SmtpAuthStartTlsIntegrationTest`에 ETRN 빈 인자/정상 인자 시나리오 추가
  - 도메인 인자가 실제 스풀러 훅으로 전달되는지 검증

### 코드 리뷰 반영(유지보수/안정화)

- `SessionData` AUTH 잠금 상태 필드를 외부 읽기 가능(`internal set`)으로 복원해 API 호환 리스크를 완화
- `ETRN` 폴백 응답을 분기해 도메인 필터 미지원 스풀러에서의 동작을 명확히 표현
